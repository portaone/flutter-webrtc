...
public class TrustedCertificateVerifierTest {
  ...
  /**
   * With nothing supplied the platform still decides, so a verifier exists - and with no
   * `turns:` endpoint to name a host for it, it can only refuse. That is the safe answer: a
   * certificate offered for a server the configuration never named is not one to accept.
   */
  @Test
  public void thePlatformDecidesWhenNothingWasSupplied() {
    TrustedCertificateVerifier verifier = TrustedCertificateVerifier.create(null);

    assertNotNull(verifier);
    assertFalse(verifier.verify(bytes(LEAF)));
    assertNotNull(TrustedCertificateVerifier.create(Collections.<byte[]>emptyList()));
  }

  /** An unreadable supplied entry is a warning, not a reason to give up on the platform. */
  @Test
  public void thePlatformStillDecidesWhenNothingSuppliedCouldBeRead() {
  ...
  /**
   * On a plain JVM the platform's hostname-aware check is a stub that throws, so it refuses
   * everything, and the endpoint here is one nothing listens on. A leaf the platform cannot
   * vouch for, from a server that teaches nothing, is refused - quickly, and without the
   * verifier giving up on the endpoint list as a whole.
   */
  @Test
  public void refusesWhatNeitherThePlatformNorTheServerCanVouchFor() {
    TrustedCertificateVerifier verifier =
        TrustedCertificateVerifier.create(null, Arrays.asList("127.0.0.1:1"));

    assertNotNull(verifier);
    long started = System.currentTimeMillis();
    assertFalse(verifier.verify(bytes(LEAF)));
    assertTrue("one unreachable endpoint must not cost the whole wait",
  ...
  /** A leaf naming no host leaves the endpoint order as given; nothing is guessed. */
  @Test
  public void endpointsKeepTheirOrderWhenTheLeafNamesNone() throws Exception {
    java.security.cert.X509Certificate leaf =
        (java.security.cert.X509Certificate)
            java.security.cert.CertificateFactory.getInstance("X.509")
                .generateCertificate(new java.io.ByteArrayInputStream(bytes(LEAF)));

    assertFalse(TrustedCertificateVerifier.names(leaf, "turn.example.com"));
    assertEquals(
        Arrays.asList("a.example.com:5349", "b.example.com:443"),
        TrustedCertificateVerifier.endpointsNamedFirst(
            leaf, Arrays.asList("a.example.com:5349", "b.example.com:443")));
  }
}
